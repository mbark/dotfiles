...
prompt_dir() {
  ...
  [[ $WILDCHERRY_DIR_EXTENDED == true ]] && dir="${dir}%6(c:…:)%5c" || dir="${dir}%1~"
...
prompt_status() {
  ...
  local symbols
  symbols=()
  if [[ $RETVAL -ne 0 && $WILDCHERRY_STATUS_EXIT_SHOW != true ]] then
    symbols+="👹  $(nice_exit_code $RETVAL)"
  fi
...
PROMPT='
%{%f%b%k%}$(build_prompt_upper)
%{%f%b%k%}$(build_prompt_downer)
